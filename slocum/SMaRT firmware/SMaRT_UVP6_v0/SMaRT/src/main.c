...
// Forward decl for stop sequence
...
void main(void){
#if DEBUG_RTT
    printk("[RTT] Main start. Polling both UARTs (no IRQs)\n");
#endif
    if (!device_is_ready(uart0_dev) || !device_is_ready(uart1_dev)) {
#if DEBUG_RTT
        printk("[RTT] ERROR: UART devices not ready\n");
#endif
...
#if DEBUG_RTT
    printk("[RTT] Waiting tokens U0='%s' U1='%s' (SD,3 epoch; SD,5 mode)\n", GLIDER_EXPECTED_STR, SENSOR_EXPECTED_STR);
#endif

    // Deadlines for initial SW report
    const int64_t t0_deadline = k_uptime_get() + GLIDER_TIMEOUT_MS;
    const int64_t t1_deadline = k_uptime_get() + SENSOR_TIMEOUT_MS;

    // Phase 1: initial window (for SW report)
    for (;;) {
        poll_drain_uart(uart0_dev, on_u0_phase1);
        poll_drain_uart(uart1_dev, on_u1_phase1);

        int64_t now = k_uptime_get();
        bool g_known = glider_seen || (now >= t0_deadline);
        bool s_known = sensor_seen || (now >= t1_deadline);
        if (g_known && s_known) break;

        k_sleep(K_MSEC(1));
    }

    // Send SW status once → SCI generic B (index 0)
    uint8_t sw_val = (glider_seen ? 1 : 0) | (sensor_seen ? 2 : 0);
    send_sw_on_uart0_uint(0, sw_val); // index 0 as requested
#if DEBUG_RTT
    printk("[RTT] SW status sent to idx0: %u (bit0=glider, bit1=sensor)\n", sw_val);
#endif
